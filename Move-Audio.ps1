...
<#
.SYNOPSIS
Moves loose audio files from Desktop and Documents to a destination folder.
.DESCRIPTION
Only processes files directly in the source folders, including hidden files.
Does not inspect project references: moving audio can break links in a DAW.
...
.EXAMPLE
.\Move-Audio.ps1 -MinAgeMinutes 120
.EXAMPLE
...
try {
    ...
    # Destination: Windows Music folder by default.
    # The second $dest line overrides the default when uncommented.
    # Change its path, or comment it out to use the Windows Music folder.
