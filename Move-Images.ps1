...
<#
.SYNOPSIS
Moves loose image files from Desktop and Documents to a destination folder.
.DESCRIPTION
Only processes files directly in the source folders, including hidden files.
RAW files and XMP sidecars are not processed.
Does not inspect project references: moving images can break links in a project.
...
.EXAMPLE
.\Move-Images.ps1 -DryRun
.EXAMPLE
.\Move-Images.ps1 -MinAgeMinutes 120
.EXAMPLE
.\Move-Images.ps1 -DryRun -MinAgeMinutes 0
.NOTES
...
try {
    ...
    # Destination: Windows Pictures folder by default.
    ...
    if (-not $DryRun) {
        ...
        Set-Content -LiteralPath $script:logFile -Value 'Image cleanup' -Encoding UTF8
        Write-Host "Log: $script:logFile"
    }

    # Only these image formats are moved. RAW and XMP files are excluded.
    $exts = @(
